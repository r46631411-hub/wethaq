...
var documents = mysqlTable("documents", {
  ...
  // بالبايت
  status: mysqlEnum("status", ["uploaded", "signed", "sealed"]).default("uploaded").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var signatures = mysqlTable("signatures", {
  id: int("id").autoincrement().primaryKey(),
  documentId: int("documentId").notNull(),
  userId: int("userId").notNull(),
  signatureImageKey: varchar("signatureImageKey", { length: 512 }).notNull(),
  // S3 key للتوقيع
  signatureImageUrl: text("signatureImageUrl").notNull(),
  // S3 URL للتوقيع
  signedAt: timestamp("signedAt").defaultNow().notNull(),
  signatureHash: varchar("signatureHash", { length: 256 }).notNull(),
  // بصمة التوقيع
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var sealedDocuments = mysqlTable("sealedDocuments", {
  id: int("id").autoincrement().primaryKey(),
  documentId: int("documentId").notNull(),
  signatureId: int("signatureId").notNull(),
  sealedDocumentKey: varchar("sealedDocumentKey", { length: 512 }).notNull(),
  // S3 key للمستند الموثق
  sealedDocumentUrl: text("sealedDocumentUrl").notNull(),
  // S3 URL للمستند الموثق
  sealHash: varchar("sealHash", { length: 256 }).notNull(),
  // بصمة الختم
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
...
async function createSealedDocument(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(sealedDocuments).values(data);
  return result;
}
async function getSealedDocumentByDocumentId(documentId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(sealedDocuments).where(eq(sealedDocuments.documentId, documentId)).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
...
var documentsRouter = router({
  ...
  uploadDocument: protectedProcedure.input(
    ...
    try {
      ...
      return {
        success: true,
        documentId: Math.floor(Math.random() * 1e4),
        fileUrl: url
      };
    } catch (error) {
  ...
  getDocument: protectedProcedure.input(z2.object({ documentId: z2.number() })).query(async ({ ctx, input }) => {
    try {
  ...
  // حفظ التوقيع
  saveSignature: protectedProcedure.input(
    z2.object({
      documentId: z2.number(),
      signatureImageData: z2.string()
      // Base64 encoded
    })
  ).mutation(async ({ ctx, input }) => {
    try {
      ...
      const signatureHash = crypto.createHash("sha256").update(buffer).digest("hex");
      ...
      return {
        success: true,
        signatureId: Math.floor(Math.random() * 1e4),
        signatureUrl
      };
    } catch (error) {
  ...
  getDocumentSignatures: protectedProcedure.input(z2.object({ documentId: z2.number() })).query(async ({ ctx, input }) => {
    try {
      const doc = await getDocumentById(input.documentId);
      if (doc && doc.userId !== ctx.user.id) {
        throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D \u0628\u0627\u0644\u0648\u0635\u0648\u0644 \u0644\u0647\u0630\u0627 \u0627\u0644\u0645\u0633\u062A\u0646\u062F");
  ...
  // ختم المستند
  sealDocument: protectedProcedure.input(
    z2.object({
      documentId: z2.number(),
      signatureId: z2.number()
    })
  ).mutation(async ({ ctx, input }) => {
    try {
      const doc = await getDocumentById(input.documentId);
      ...
      const sealHash = crypto.createHash("sha256").update(`${input.documentId}-${input.signatureId}-${Date.now()}`).digest("hex");
      const sealedDocumentKey = `sealed-documents/${ctx.user.id}/${input.documentId}-${Date.now()}.pdf`;
      const sealedDocumentUrl = doc.fileUrl;
      ...
      return {
        success: true,
        sealedDocumentId: Math.floor(Math.random() * 1e4),
        sealedDocumentUrl,
        sealHash
      };
    } catch (error) {
  ...
  getSealedDocument: protectedProcedure.input(z2.object({ documentId: z2.number() })).query(async ({ ctx, input }) => {
    try {
      const doc = await getDocumentById(input.documentId);
      if (doc && doc.userId !== ctx.user.id) {
        throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D \u0628\u0627\u0644\u0648\u0635\u0648\u0644 \u0644\u0647\u0630\u0627 \u0627\u0644\u0645\u0633\u062A\u0646\u062F");
